...
import com.khaizro.blnotifly.utils.BluetoothUIHelper
import com.khaizro.blnotifly.utils.MainActivityUIHelper
import com.khaizro.blnotifly.utils.NotificationHelper
import com.khaizro.blnotifly.utils.PermissionManager
...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleEvent(event: String) {
        Log.d("BLNotiflyMA", "handleEvent: $event")
        when {
    ...
    private fun showRemote(event: String) {
        ...
        Log.d("BLNotiflyMA", "showRemote rawData: $rawData")
        // Split by newlines if multiple messages were bundled, or just handle the single message
        rawData.split("\n")
            .map { it.trim() }
            .filter { it.isNotBlank() }
            .forEach { line ->
                Log.d("BLNotiflyMA", "Parsing line: $line")
                NotificationPayload.parse(line)?.let { payload ->
                    Log.d("BLNotiflyMA", "Showing notification for: ${payload.appName}")
